...
static Boolean MySimpleUpload(CFStringRef uploadDirectory, CFURLRef fileURL, CFStringRef username, CFStringRef password)
{
	...
	//readStream = CFReadStreamCreateWithBytesNoCopy(kCFAllocatorDefault, <#const UInt8 *bytes#>, <#CFIndex length#>, <#CFAllocatorRef bytesDeallocator#>);
...
- (void) put: (NSURL *) fileLocation
{	
	if (!self.uname || !self.pword) COMPLAIN_AND_BAIL(@"Please set user name and password first");
	if (!self.urlString) COMPLAIN_AND_BAIL(@"Please set URL string first");
	MySimpleUpload((__bridge CFStringRef)self.urlString, (__bridge CFURLRef)fileLocation, (__bridge CFStringRef)uname, (__bridge CFStringRef)pword);
}

- (void) updateByData: (NSData *) fileData fileName:(NSString *)imageName
{	
	if (!self.uname || !self.pword) COMPLAIN_AND_BAIL(@"Please set user name and password first");
	if (!self.urlString) COMPLAIN_AND_BAIL(@"Please set URL string first");
	...
	MySimpleUploadByData((__bridge CFStringRef)self.urlString, fileData, (__bridge CFStringRef)uname, (__bridge CFStringRef)pword, imageName);
}

+ (void) upload: (NSURL *) anItem
{
...
{ 
	[sharedInstance updateByData:anItem fileName:imageName];
}
...
- (void) fetch: (NSString *) anItem
{
	...
	NSString *writepath = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/CSVfile"];
    NSLog(@"%@",writepath);
